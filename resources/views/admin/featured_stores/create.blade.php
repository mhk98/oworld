...
@section('title', 'Add New Store | '.env('APP_NAME'))
@section('content')
...
<form action="{{ route('admin.featured-stores.store') }}" method="post" enctype="multipart/form-data" id="postContent">
    @csrf
    <div class="row g-3">
        <div class="col-md-9">
            <div class="card my-2">
                ...
                <div class="card-body p-2">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('serial') is-invalid @enderror" name="serial" required>
                            @for ($i = 1; $i <= 20; $i++) <option value="{{ $i }}" {{ old('serial') == $i ? 'selected' : '' }}>{{ $i }}</option>
                                @endfor
                        </select>
                        @error('serial')
                        <span class="invalid-feedback" role="alert">
