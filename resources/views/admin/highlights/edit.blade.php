...
@section('title', 'Edit Highlight | '.env('APP_NAME'))
@section('content')
<nav class="page-breadcrumb">
    <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="{{ route('admin.highlights.index') }}">Highlights</a></li>
        <li class="breadcrumb-item active" aria-current="page">Edit Highlight</li>
    </ol>
</nav>
        ...
        <div class="col-md-9">
            <div class="card my-2">
                <div class="card-head p-2 border-bottom">
                    <h6 class="card-title my-1">Edit Highlight</h6>
                </div>
                <div class="card-body p-2">
                    <div class="image-box mb-1">
                        <label class="form-label">Image </label>
                        <div class="image-box-preview">
                            <img id="highlight_image" src="{{ !empty($highlight->image) ? asset('media/'.$highlight->image) : asset('static/admin/images/default.jpg') }}" alt="Highlight Image">
                        </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('store_id') is-invalid @enderror" name="store_id" required>
                            ...
                            <option value="{{ $store->id }}" {{ $highlight->store_id == $store->id ? 'selected' : '' }}>
                                {{ $store->business_name }}
                            </option>
                            @endforeach
                            @endif
                    ...
                    <div class="mb-3 mt-4">
                        ...
                        <textarea name="description" cols="10" rows="3" class="form-control">{{ $highlight->description }}</textarea>
                        @error('description')
                    ...
                    <div class="mb-3">
                        ...
                        <input type="date" class="form-control @error('published_date') is-invalid @enderror" name="published_date" value="{{ $highlight->published_date }}" required>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('status') is-invalid @enderror" name="status" required>
                            <option value="pending" {{ $highlight->status == 'pending' ? 'selected' : '' }}>
                                Pending
                            </option>
                            <option value="published" {{ $highlight->status == 'published' ? 'selected' : '' }}>
                                Published
                            </option>
                            <option value="private" {{ $highlight->status == 'private' ? 'selected' : '' }}>
                                Private
                            </option>
                        </select>
                        @error('status')
