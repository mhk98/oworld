...
@section('title', 'Edit Billboard | '.env('APP_NAME'))
@section('content')
<nav class="page-breadcrumb">
    <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="{{ route('admin.billboards.index') }}">Billboards</a></li>
        <li class="breadcrumb-item active" aria-current="page">Edit Billboard</li>
    </ol>
</nav>
        ...
        <div class="col-md-9">
            <div class="card my-2">
                <div class="card-head p-2 border-bottom">
                    <h6 class="card-title my-1">Edit Billboard</h6>
                </div>
                <div class="card-body p-2">
                    <div class="image-box mb-1">
                        <label class="form-label">Image </label>
                        <div class="image-box-preview">
                            <img id="billboard_image" src="{{ !empty($billboard->image) ? asset('media/'.$billboard->image) : asset('static/admin/images/default.jpg') }}" alt="Billboard Image">
                        </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('store_id') is-invalid @enderror" id="store" name="store_id" required>
                            ...
                            <option value="{{ $store->id }}" {{ $billboard->store_id == $store->id ? 'selected' : '' }}>
                                {{ $store->business_name }}
                            </option>
                            @endforeach
                            @endif
                ...
                <input type="date" class="form-control @error('published_date') is-invalid @enderror" name="published_date" value="{{ $billboard->published_date }}" required>
                ...
                @enderror
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('serial') is-invalid @enderror" name="serial" id="serial" required>
                            @for($i = 1; $i <= 10; $i++) 
                            <option value="{{ $i }}" {{ $billboard->serial == $i ? 'selected' : '' }}>
                                {{ $i }}
                            </option>
                            @endfor
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('status') is-invalid @enderror" name="status" required>
                            <option value="pending" {{ $billboard->status == 'pending' ? 'selected' : '' }}>Pending</option>
                            <option value="private" {{ $billboard->status == 'private' ? 'selected' : '' }}>Private</option>
