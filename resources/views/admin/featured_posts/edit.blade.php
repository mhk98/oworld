...
@section('title', 'Edit Featured Post | '.env('APP_NAME'))
@section('content')
<nav class="page-breadcrumb">
    <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="{{ route('admin.featured-posts.index') }}">Featured Posts</a></li>
        <li class="breadcrumb-item active" aria-current="page">Edit Featured Post</li>
    </ol>
</nav>
<form action="{{ route('admin.featured-posts.update', $featuredPost->id) }}" method="post" enctype="multipart/form-data" id="updatePost">
    @csrf
    @method('PUT')
    <div class="row g-3">
        <div class="col-md-9">
            <div class="card my-2">
                ...
                <div class="card-body p-2">
                    ...
                    @if ($featuredPost->images && count($featuredPost->images) > 0)
                    <div class="row gallery-container g-3 mb-3">
                        @foreach($featuredPost->images as $imageId => $image)
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('title') is-invalid @enderror" name="title" value="{{ old('title', $featuredPost->title) }}" required>
                        @error('title')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('description') is-invalid @enderror" name="description" rows="3">{{ old('description', $featuredPost->description) }}</textarea>
                        @error('description')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('featured_section_id') is-invalid @enderror" name="featured_section_id">
                            ...
                            <option value="{{ $featuredSection->id }}" {{ $featuredPost->featured_section_id == $featuredSection->id ? 'selected' : '' }}>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('store_id') is-invalid @enderror" id="stores-select" name="store_id">
                            ...
                            <option value="{{ $store->id }}" {{ $featuredPost->store_id == $store->id ? 'selected' : '' }}>
                                {{ $store->business_name }}
                            </option>
                            @endforeach
                            @endif
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('serial') is-invalid @enderror" name="serial" required>
                            @for ($i = 1; $i <= 20; $i++) <option value="{{ $i }}" {{ $featuredPost->serial == $i ? 'selected' : '' }}>
                                {{ $i }}
                                </option>
                                @endfor
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('status') is-invalid @enderror" name="status" required>
                            <option value="active" {{ old('status', $featuredPost->status) == 'pending' ? 'selected' : '' }}>Active</option>
                            <option value="deactive" {{ old('status', $featuredPost->status) == 'pending' ? 'selected' : '' }}>Deactive</option>
                        </select>
                        @error('status')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
...
<script>
    $(document).ready(function() {
        ...
        $(document).on('click', '#removeImageItem', function(event) {
            ...
            var postId = '{{ $featuredPost->id }}';
            var confirmation = confirm("Are you sure you want to delete this image?");

            if (confirmation) {
                $.ajax({
                    url: '{{ route("admin.featuredPostsRemoveImage") }}',
                    type: 'DELETE',
                    data: {
                        _token: '{{ csrf_token() }}',
                        postId: postId,
